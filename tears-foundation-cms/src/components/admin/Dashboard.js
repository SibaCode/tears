// src/pages/admin/Dashboard.js - Update to handle both admin and counsellor views
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    // Fetch help requests (admins only)
    if (userRole === 'admin') {
    ...
    <div style={{padding: 'var(--spacing-6)'}}>
      <div style={{marginBottom: 'var(--spacing-6)'}}>
        ...
        <p style={{color: 'var(--secondary-gray)'}}>
          {userRole === 'admin' 
            ? 'TEARS Foundation Case Management System' 
            : `Welcome back, ${userData?.name || 'Counsellor'}`}
        </p>
      </div>
      ...
      <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 'var(--spacing-6)', marginBottom: 'var(--spacing-8)'}}>
        <div className="card">
          <div className="card-body" style={{textAlign: 'center'}}>
            <h3 style={{fontSize: 'var(--font-size-sm)', color: 'var(--secondary-gray)', marginBottom: 'var(--spacing-2)'}}>
              {userRole === 'counsellor' ? 'My Cases' : 'Total Cases'}
            </h3>
      ...
      {/* Help Requests Alert (Admin only) */}
      ...
      <div className="card">
        <div className="card-header" style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
          <h2>{userRole === 'counsellor' ? 'My Recent Cases' : 'Recent Cases'}</h2>
          <Link 
            to="/admin/cases"
              ...
              <tbody>
                {cases.slice(0, 5).map((caseItem) => (
                  <tr key={caseItem.id}>
                    ...
                    <td className="table-cell">
                      <span className={`badge ${
                        caseItem.urgency === 'high' ? 'badge-high' :
                        caseItem.urgency === 'medium' ? 'badge-medium' :
                        'badge-low'
                      }`}>
                        {caseItem.urgency}
                      </span>
